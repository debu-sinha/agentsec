# punkpeye/fastmcp

![Grade](https://img.shields.io/badge/Grade-A-brightgreen?style=for-the-badge) ![Score](https://img.shields.io/badge/Score-92%2F100-brightgreen?style=for-the-badge)

**Repository:** [punkpeye/fastmcp](https://github.com/punkpeye/fastmcp)
**Stars:** 3,194
**Last scan:** 2026-06-15

## Severity Summary

| Severity | Count |
|----------|------:|
| 🟢 Low | **8** |
| 🔵 Info | **1** |
| **Total** | **9** |

## Findings

| # | Severity | Category | Title | Remediation |
|--:|:--------:|----------|-------|-------------|
| 1 | 🔵 Info | Outdated Version | Could not determine agent version | Ensure agent is updated to latest version |
| 2 | 🟢 Low | Exposed Token | Secret Keyword found in FastMCP.oauth-proxy.test.ts | Rotate and secure the Secret Keyword |
| 3 | 🟢 Low | Exposed Token | Secret Keyword found in tokenStore.test.ts | Rotate and secure the Secret Keyword |
| 4 | 🟢 Low | Exposed Token | Private Key found in server-key.pem | Rotate and secure the Private Key |
| 5 | 🟢 Low | Exposed Token | Secret Keyword found in OAUTH-PROXY.md | Rotate and secure the Secret Keyword |
| 6 | 🟢 Low | Exposed Token | Secret Keyword found in OAuthProxy.token-swap.test.ts | Rotate and secure the Secret Keyword |
| 7 | 🟢 Low | Exposed Token | Secret Keyword found in oauth-python-typescript.md | Rotate and secure the Secret Keyword |
| 8 | 🟢 Low | Exposed Token | Secret Keyword found in oauth-proxy-guide.md | Rotate and secure the Secret Keyword |
| 9 | 🟢 Low | Exposed Token | Base64 High Entropy String found in FastMCP.test.ts | Rotate and secure the Base64 High Entropy String |

## Categories

| Category | Count |
|----------|------:|
| Exposed Token | 8 |
| Outdated Version | 1 |

---

[Back to Dashboard](../mcp-security-grades.md) | *Scanned on 2026-06-15 by [agentsec](https://github.com/debu-sinha/agentsec)*
